        ...
        <div v-if="loading">
          <div
            class="absolute z-10 h-full w-full animate-pulse
              bg-(--_mediaColor)/50 transition-[opacity,background]"
          ></div>
        </div>
      </TransitionFade>
      <TransitionFade>
        <template v-if="visible">
          <div v-if="mediaType === 'icon'" class="absolute h-full w-full">
            <div
              class="grid size-full place-items-center bg-(--_mediaColor)/30
