...
import type { DiagramSchema } from './diagram.global';
import Caption from '../caption/Caption.vue';
import { useProseAppContext } from '../../app';
import { useIcon } from '../../app/front/composables/icon';
import { usePhotoSwipe } from '../../shared';

const { element } = defineProps<{ element: ParsedElement<DiagramSchema> }>();
const style = useCssModule();

const Icon = useIcon();
const { loadingSvg } = useProseAppContext();
const loading = ref(true);
const diagramRendering = ref(false);
const dummyMathHtml = ref('');
const diagramContent = ref(element.data);
