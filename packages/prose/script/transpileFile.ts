...
function ensureJsExtensions(code: string, fileDir: string): string {
  ...
  const fixPath = (spec: string) => {
    ...
    if (/\.(ts|tsx|js)$/i.test(spec))
      return spec.replace(/\.(ts|tsx|js)$/i, '.js');
...
export function transpileFile(
  ...
  if (ext === '.ts' || ext === '.tsx') {
    ...
    const jsOutPath = `${outRoot}/${relPath.replace(/\.(ts|tsx)$/, '.js')}`;

    mkdirSync(dirname(jsOutPath), { recursive: true });
    let jsCode = result.code;
    jsCode = ensureJsExtensions(jsCode, dirname(file));
