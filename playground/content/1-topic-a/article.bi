...
@important
    ...
    main: |
        ...
        * Прикидываем [комбинации](~bar) дел и задач, занимаясь планированием.
        * Пишем комбинации букв, получая слова.
        * Набираем комбинации цифр, чтобы кому-нибудь позвонить.
        * Выстраиваем комбинации мыслей, образуя рассуждения.

~> page|article|/test/link

~> page|article|/test/link Привет мир, с Вами Джонни! И Сегодня мы будем не по детски тусить!
...
Это большая и сложная задача, которую вам нужно решить!
...
@important
    ...
    main: |
        Избавляемся от смысла
        Нас не интересует, какой бытовой смысл стоит за комбинациями.
        Комбинаторика рассматривает только вопросы, связанные с **составлением**{ accent } комбинаций!
...
@term
    ...
    main: |
        Функция $f(x)$ на промежутке $M\subseteq \R$ называется **строго возрастающей**{ accent }, когда для любых двух чисел $x_1, x_2 \in M$ этого промежутка, таких, что $x_1 < x_2$, выполняется неравенство $f(x_1) < f(x_2)$.
...
@example
    ...
    sections:
        "Пункт а)": |
            Берем две [произвольные](~bar) точки $x_1$ и $x_2$, такие что $x_1 < x_2$. Раз $f(x) = x$, то получаем следующую ситуацию:
